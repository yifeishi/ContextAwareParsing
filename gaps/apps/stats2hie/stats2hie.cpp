...
char *output_room_filename = NULL;
static char *room_type = NULL;
...
ReadStats(char *filename)
{
	...
	// Read scene from file
	if (!scene->ReadStatsFile(filename)) {
		delete scene;
		return NULL;
	}
...
static int
WriteRoom(char *scene_name, char *room_type, char *filename)
{
  // Start statistics
  RNTime start_time;
  start_time.Read();
  ...
  // Write file
  if (!scene->WriteRoomFile(scene->Root()->Name(), room_type, filename, scene, scene->Root())) return 0;

  // Print statistics
  if (print_verbose) {
    printf("Write Room Info to %s ...\n", filename);
    printf("  Time = %.2f seconds\n", start_time.Elapsed());
    fflush(stdout);
  }
...
int main(int argc, char **argv)
{
  ...
  // Read scene
  fprintf(stderr, "ReadScene\n \n");
  scene = ReadScene(input_scene_name);
  if (!scene) exit(-1);

  // Read stats
  fprintf(stderr, "ReadStats\n \n");
  scene = ReadStats(input_stats_name);
  if (!scene) exit(-1);
